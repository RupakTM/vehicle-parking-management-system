                            ...
                            <table class="table table-bordered">
                                ...
                                @forelse ($data['rows'] as $index => $row)
                                    <tr>
                                        ...
                                        <td>
                                            <form action="{{route('role.restore',$row->id)}}" method="post" class="d-inline">
                                                @csrf
                                                <button type="submit" class="btn btn-success">
                                                    <i class="fa fa-recycle" aria-hidden="true"></i> Restore</button>
                                            </form>
                                            <form action="{{route('role.forceDelete',$row->id)}}" method="post" class="d-inline">
                                                @csrf
                                ...
                                @empty
                                    <tr>
                                        <td colspan="5" class="text text-danger">Role not found</td>
                                    </tr>
                                @endforelse
                                </tbody>
                            </table>
